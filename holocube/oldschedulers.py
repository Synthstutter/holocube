...
class Scheduler():
    ...
    def start(self, window, lmr, rest_bar, rest_time=5.0, randomize=True,
                 freq=120):

        self.lmr = lmr
        self.rest_bar = rest_bar
        self.freq = freq
    ...
    def rest(self, dt=0.0):
        self.rest_bar.inc_ry(-self.lmr()*.3)
    ...
    def rest_to_test(self, dt=0.0, test_num=0):
        ...
        self.curr_test_start =    [test for test in curr_test if test[2]== 0]
        self.curr_test_end   =    [test for test in curr_test if test[2]==-1]
        self.curr_test_interval = [test for test in curr_test if test[2]== 1 and hasattr(test[1], '__iter__')]
        self.curr_test_funcs    = [test for test in curr_test if test[2]== 1 and hasattr(test[1], '__call__')]
    ...
    def begin(self, state=True):
        if state:
            if self.randomize:
                ...
                self.order = arange(len(self.tests))
            self.curr_num = 0
            print 'begin ##########################################################'
            self.test_to_rest()
        else:
